...
PNODE findList(PDLIST lst, int k, int i) {
	PNODE nd = lst->left;
	while(nd != NULL && (nd->key != k || nd->info != i)) {
		nd = nd->next;
	}
	return nd;
}
...
void LINUX_printList(PDLIST lst) {
	...
	while (nd != NULL) {
		switch (nd->info) {
		case 0: printf("[\xE2\x99\xA0"); break;
		case 1: printf("[\xE2\x99\xA3"); break;
		case 2: printf("[\xE2\x99\xA5"); break;
		case 3: printf("[\xE2\x99\xA6");break;
		}
		if (nd->key > 9) {
			switch (nd->key) {
			case 10: printf(" 10]\n"); break;
